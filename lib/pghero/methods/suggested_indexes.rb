        ...
        if !citus_enabled?
          ...
          select_all <<-SQL
            ...
            (
              ...
              CROSS JOIN
                json_to_recordset(pg_dist_stats_single_json.result::json) AS y("tablename" name, "attname" name, "null_frac" real, "reltuples" integer, "n_distinct" real) 
            ),
            pg_dist_stats AS (
              SELECT
            ...
            pg_dist_n_distinct AS (
              SELECT 
                tablename,
                attname,
                CASE WHEN t.n_distinct >= 0 THEN t.n_distinct ELSE (-1) * t.n_distinct * t.reltuples END AS n_distinct
              FROM
                pg_dist_stats_regular t 
              JOIN
                pg_class ON relname = tablename 
            ...
            SELECT
              ...
              CASE WHEN ((
                ...
                THEN 
                  CASE WHEN (SELECT column_to_column_name(logicalrelid, partkey) FROM pg_dist_partition WHERE logicalrelid::name=s.tablename) = s.attname
                  THEN (
                    SELECT
                      sum(n_distinct)::int
                    FROM
                      pg_dist_n_distinct n
                    WHERE
                      n.tablename = s.tablename AND n.attname = s.attname
                    )
                  ELSE (
                    SELECT
                      max(n_distinct)::int
