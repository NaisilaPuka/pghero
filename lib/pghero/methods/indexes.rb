      ...
      def index_bloat(min_size: nil)
        ...
        if !citus_enabled?
          ...
          select_all <<-SQL
            ...
            pg_dist_all_placements_stats_agg AS (
              SELECT (
                SELECT
                  json_agg(row_to_json(all_placements_stats))
                FROM (
                  SELECT
                    result
                  FROM
                    run_command_on_placements(logicalrelid, $$
                      SELECT
                        json_agg(row_to_json(placement_stats))
                      FROM (
                        SELECT
                ...
                ) all_placements_stats)
              FROM
                pg_dist_partition
            ),
            pg_dist_placement_stats_agg AS (
              SELECT
                (json_array_elements(json_agg)->>'result') AS result
              FROM
                pg_dist_all_placements_stats_agg
            ),
            pg_dist_placement_stats AS (
              SELECT
            ...
            index_item_sizes AS (
              ...
              JOIN (
                  SELECT
                    ...
                    attname,
                    sum(null_frac * ds.reltuples) / sum(ds.reltuples) AS null_frac,
                    sum(avg_width * ds.reltuples) / sum(ds.reltuples) AS avg_width
                  FROM
                    pg_dist_placement_stats ds
                  JOIN
